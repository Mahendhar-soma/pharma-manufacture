...
export default function HospitalsPage() {
  ...
  const [form, setForm] = useState({ hospital_name: "", city: "", phone: "" });
  ...
  async function create(e: React.FormEvent) {
    ...
    try {
      ...
      setShowForm(false);
      setForm({ hospital_name: "", city: "", phone: "" });
      reload();
    } catch (err) {
  ...
  return (
    <AppLayout title="Hospitals">
      <PageHeader
        ...
        actions={<Button onClick={() => setShowForm((v) => !v)}>{showForm ? "Close" : "New Hospital"}</Button>}
      />
      {showForm ? (
        <Card className="mb-4">
