...
export default function DoctorsPage() {
  ...
  const [form, setForm] = useState({ doctor_name: "", specialization: "", city: "", phone: "" });
  ...
  async function create(e: React.FormEvent) {
    ...
    try {
      ...
      setShowForm(false);
      setForm({ doctor_name: "", specialization: "", city: "", phone: "" });
      reload();
    } catch (err) {
  ...
  return (
    <AppLayout title="Doctors">
      <PageHeader
        ...
        actions={<Button onClick={() => setShowForm((v) => !v)}>{showForm ? "Close" : "New Doctor"}</Button>}
      />
      {showForm ? (
        <Card className="mb-4">
